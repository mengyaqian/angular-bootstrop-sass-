...
export class OvertimeComponent implements OnInit {
  ...
  tableCurreny:number=1; //当且tab 请假汇总1，请假明细2
 ...
 //请假汇总统计
  ...
  importCon(){
    ...
    if(this.tableCurreny == 1){//请假汇总
        let openUrl = 'http://10.0.0.14:8080/yodooweb/leaveBillReport/export?startdate='+this.startdate+'&enddate='+this.enddate+'&departId='+this.departId+'&username='+this.username+'&type=0'; 
        window.location.href=openUrl;
      }else{//请假明细
        let openUrl = 'http://10.0.0.14:8080/yodooweb/leaveBillReport/exportDetail?startdate='+this.startdate+'&enddate='+this.enddate+'&departId='+this.departId+'&username='+this.username+'&type=0'; 
        window.location.href=openUrl;
      }
  }
   //重置
